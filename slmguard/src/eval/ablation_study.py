...
from model.train import SLMGuardModel, SLMGuardTrainer, preprocess_function, HARM0_WEIGHT
...
class AblationConfig:
    ...
    alpha:        float = 0.7      
    use_focal:    bool  = True     
    harm0_weight: float = HARM0_WEIGHT  
    freeze_epochs:int   = 1        
    epochs:       int   = 5
    batch_size:   int   = 16
    lr:           float = 2e-5
...
ABLATIONS = {
    ...
    "A7": AblationConfig(
        name="A7 — Middle-ground alpha",
        description="alpha=0.85, between A1's binary-only (1.0) and A3's proposed (0.7). "
                    "Tests whether a smaller multiclass weight recovers most of A1's binary "
                    "performance while still training a usable explanation head.",
        alpha=0.85,
    ),
}
