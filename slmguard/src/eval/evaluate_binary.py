...
from model.config import ALL_LABELS
...
def load_model(ckpt_dir):
    ...
    if cfg_path.exists():
        ...
        model_key  = cfg.get('model_key', 'modernbert')
...
def evaluate(ckpt_dir, split='test', max_samples=None, threshold=0.5):
    ...
    for ex in ds:
        ...
        true_bin.append(int(ex['harm_label']))
        preds_mc.append(out['multiclass_logits'].argmax(-1).item())
        true_mc.append(int(ex['label_id']))
    ...
    print(f"{'='*50}")
    print("\nClassification Report (binary, main task):")
    print(classification_report(true_bin, preds_bin, target_names=['harm=0', 'harm=1']))

    mc_acc      = accuracy_score(true_mc, preds_mc)
    mc_macro_f1 = f1_score(true_mc, preds_mc, average='macro', zero_division=0)
    print(f"\n{'='*50}")
    print(f"  Auxiliary head (12-way SE subtype)")
    print(f"{'='*50}")
    print(f"  Accuracy   : {mc_acc*100:.2f}%")
    print(f"  Macro F1   : {mc_macro_f1:.4f}")
    print("\nClassification Report (multiclass, auxiliary task):")
    print(classification_report(
        true_mc, preds_mc, labels=list(range(len(ALL_LABELS))),
