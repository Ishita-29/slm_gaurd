...
from model.config import ID2LABEL, DEPLOY_CKPT

# ─── Paths ────────────────────────────────────────────────────────────────────
GUARD_CKPT = DEPLOY_CKPT
# ood_judged/ (not ood/) — attack/benign ground truth here is a judged harm_label
# (see harm_label_judge.py), not the original is_se field, which meant different
# things across sources and isn't a valid harm proxy.
OOD_DIR    = '/data/ishita_workspace/SLM-GAURD/slmguard/data/ood_judged'
...
def load_ood_samples(source_key, n_attacks=100, n_benign=50, seed=42):
    ...
    if source_key == 'all':
        ...
        for key, path in OOD_SOURCES.items():
            ...
            all_attacks.extend([d for d in data if d.get('harm_label') == 1])
            all_benign.extend([d for d in data if d.get('harm_label') == 0])
        ...
        data = [json.loads(l) for l in open(path)]
        attacks = [d for d in data if d.get('harm_label') == 1]
        benign  = [d for d in data if d.get('harm_label') == 0]
...
def evaluate(target_key, source_key, n_attacks, n_benign, threshold, guard_device, llm_device):
    ...
    attacks_s = [s for s in samples if s.get('harm_label') == 1]
    benigns_s = [s for s in samples if s.get('harm_label') == 0]
    ...
    for i, ex in enumerate(samples):
        text  = ex['text']
        harm_label = ex.get('harm_label', 0)
        ...
        status = f"[{'ATK' if harm_label else 'BEN'}] blocked={blocked} | noguard_refused={refused_noguard} | withguard_refused={refused_withguard}"
        print(f"  [{i+1:3d}/{len(samples)}] {src[:20]:20s}  {status}")

    # ── Report ─────────────────────────────────────────────────────────────
    attacks_r = [r for r in results if r['harm_label'] == 1]
    benigns_r = [r for r in results if r['harm_label'] == 0]
