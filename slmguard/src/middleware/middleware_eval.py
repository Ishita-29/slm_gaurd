...
def evaluate(target_key, n_per_subtype, n_benign, guard_threshold, guard_device, llm_device):
    ...
    n_attacks = sum(1 for s in samples if s['harm_label'] == 1)
    n_benign_s = sum(1 for s in samples if s['harm_label'] == 0)
    ...
    for i, ex in enumerate(samples):
        text    = ex['text']
        harm_label   = ex['harm_label']
        label   = ID2LABEL.get(ex['label_id'], 'unknown')

        
        blocked, prob = guard_score(guard, guard_tok, text, guard_device, guard_max_length, guard_threshold)
    ...
    attacks = [r for r in results if r['harm_label'] == 1]
    benigns = [r for r in results if r['harm_label'] == 0]
